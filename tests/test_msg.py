...
class TestMessageFunctions:
    ...
    def test_get_msg_template_no_warning(self):
        assert get_msg_template("user_id", "reason", warning=False) == """ User:<@user_id> has been banned for reason."""
        assert get_msg_template("user_id", ["reason1", "reason2"], warning=False) == """ User:<@user_id> has been banned for reason1,reason2."""
